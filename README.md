...
There are three ways to set up the bridge. Pick whichever suits you:

### Option A: Setup via Claude Code CLI (Recommended)

If you already have [Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code) installed, this is the easiest way. Claude walks you through everything interactively — installs prerequisites, configures environment variables, sets up Teams notifications, and starts the server.
...
2. Check and install prerequisites (Node.js, Claude CLI, devtunnel, cloudflared)
3. Run `npm install`
4. Start the Cloudflare tunnel (if you want Teams integration)
5. Walk you through Teams Outgoing/Incoming Webhook creation
6. Generate VAPID keys for web push notifications
7. Install the Teams notification hook (notifies you when Claude stops or asks a question)
8. Send a test notification to verify everything works
9. Start the bridge and show you the QR code + public URL

> This is a one-time setup. After that, just use `npm start` or `npm run start:tunnel`.

### Option B: Setup Script

Run the platform-specific setup script — it checks prerequisites, installs anything missing, and starts the bridge:
...
See [Manual Setup](#manual-setup) below if you prefer to install everything yourself step by step.
...
> **Why is Cloudflare required for Teams?** Microsoft devtunnel shows an anti-phishing interstitial page (a "click to continue" wall) on every first visit. This is fine for browsers where a human clicks through, but it **breaks server-to-server webhook calls** — Teams can't click through an interstitial, so outgoing webhook requests silently fail. Cloudflare's quick tunnel (`trycloudflare.com`) forwards HTTP requests directly without any interstitial, making it the right fit for the Teams webhook endpoint. The web UI itself still uses devtunnel (which handles authentication and QR codes nicely), so both tunnels run side by side: **devtunnel for the browser UI, Cloudflare for the Teams webhook**.
...
The setup script prompts for all required values and saves them as environment variables (User env vars on Windows, shell profile on macOS/Linux). No `.env` file needed.

You can also use a `.env` file if you prefer file-based config:
...
| `CLAUDE_TEAMS_WEBHOOK_URL` | — | Webhook URL for Claude Code CLI notification hook |

> Most users don't need to configure anything manually. The setup script handles it all.
...
Get notified in Microsoft Teams when Claude stops, asks for permission, or needs a decision — so you don't have to watch the terminal.

The setup script offers to configure this automatically. To set it up separately:
